...
int
AxmlToXml(char **outbuf, size_t *outsize, const char *inbuf, size_t insize)
{
	void *axml;
	AxmlEvent_t event;
	Buff_t buf;
	...
	while((event = AxmlNext(axml, 0)) != AE_ENDDOC)
	{
		char *prefix;
		char *name;
		char *value;
